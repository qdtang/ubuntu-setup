...
def exit(status, updated, path=os.curdir):
	if updated:
		dumpPaths()
	if status == 0:
		changeDir(path)
...
def usage():
   print('''
	   Please add following alias (for csh) to use this script:
	   alias jd "/path/to/this/script \!* && source ~/.jumpdir.exe"
	   Thus jd allows you to jump among directories via keywords in the path.
